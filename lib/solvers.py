...
def broyden(f, x0, threshold, eps=1e-8, stop_mode="abs", ls=False, layer_loss=True, layer_idx=[10], name="unknown"):
  dev = x0.device
  if len(x0.shape) == 4:
    init_shape = x0.shape
    bsz, C, H, W = x0.shape
  ...
  g = lambda y: f(y.view(init_shape)).view(bsz, -1, 1) - y
  ...
  protect_thres = (1e3 if stop_mode == "abs" else 1e3) * seq_len
  new_objective = 1e8
  ...
  return {"result": lowest_xest.view(init_shape),
...
def anderson(
    f, x0, m=5, lam=1e-4, threshold=50, 
    eps=1e-3, stop_mode='rel', beta=1.0, 
    layer_idx=[10], **kwargs):
  eps=1e-9
  alternative_mode = 'rel' if stop_mode == 'abs' else 'abs'
  ...
  if len(x0.shape) == 4:
    ...
    raise ValueError(f"Does not support shape {x0.shape}")
  lowest_xest = x0
